...
#define LORA_appEUI "C763721FE1A1CCC5"
#define LORA_appKEY "FCD3EEE936CA0DC0F0608A92BD0B24DD"
//#include "bits.h"
#define BIT_2 ( 1 << 2 )
...
void lora_handler_task( void *pvParameters )
{
	...
	for(;;)
	{
		...
		// Some dummy payload
		uint16_t hum = get_hum_lvl(); // not Dummy humidity
		int16_t temp = get_temp_lvl(); // not Dummy temp
		uint16_t co2_ppm = getCO2lvl(); // not Dummy CO2
		printf("%s%i\n", "Co2 value in arduino: ",co2_ppm);
		
		//uint16_t hum = 10; // not Dummy humidity
		//int16_t temp = 10; // not Dummy temp
	//	uint16_t co2_ppm = 10; // not Dummy CO2
