...
void taskControl(void* pvParameters) {
	...
	for (;;){
		...
		pdFALSE,
		300);
		if (xTestSemaphore,1000) {
			printf("Checking if all tasks are set to work\n");
			if ((uxBits & (bits)) == (bits)){
			...
			{
				puts("CO2 is not yet Healthy");
			}
			else if (!(uxBits & BIT_1))
			{
				puts("Humidity/Temperature sensor is not yet Healthy");
			}
			else if (!(uxBits & BIT_2))
			{
				puts("Lora transceiver is not yet Healthy");
			}
			else if (!(uxBits & BIT_4))
			{
				puts("Task1 is not yet Healthy");
			}
			else if (!(uxBits & BIT_5))
			{
				puts("Task2 sensor is not yet Healthy");
			}
			
			
			//else if (!(uxBits & BIT_3))
		//	{
		//		puts("Servo  is not yet Healthy");
		//	}
			
			else{puts("System is unhealthy!");}
...
void task2( void *pvParameters )
{
	//xEventGroupSetBits(xCreatedEventGroup, BIT_5);
	#if (configUSE_APPLICATION_TASK_TAG == 1)
	// Set task no to be used for tracing with R2R-Network
